...
Recall the real proper velocity $U=\dd X/\dd s=\gamma(1+\bv\cdot\bsig)$,
a \hyperref[eq:unit-paravector]{unit paravector} with $UU^{-1}=1$,
and real momentum $P=mU=E+\bp\cdot\bsig$, with constant mass $m>0$.
...
\end{equation}
This retains the electric-source laws
...
patched, singular, or enlarged potentials~\cite[pp.~6--8]{tong-monopoles}.
...
Recall the real proper velocity $U=\dd X/\dd s$, a
\hyperref[eq:unit-paravector]{unit paravector} with $UU^{-1}=1$,
real charges $q,q'$, mass $m>0$, and momentum $P=mU$.
For constant complex electromagnetic field $F$, define the complex
\hyperref[eq:unit-paravector]{unit paravector}
\begin{equation}
 T(s)=\exp\!\left(\frac{(q-\ii q')s}{2m}F\right).
 \label{eq:constant-field-factor}
\end{equation}
The solution for future real
initial momentum $P(0)$ with $\det P(0)=m^2$ is
\begin{equation}
 P(s)=T(s)P(0)T(s)^{\mathsf H}.
 \label{eq:real-force-congruence-solution}
\end{equation}
Differentiating, with real $U=P/m$, gives
\begin{align*}
 \frac{\dd P}{\dd s}
...
For $q'=0$, \hyperref[def:real-unit-vector]{real unit vector} $\bu$, and constant real field magnitudes $E,B$,
\begin{align*}
...
The evolution changes momentum in a fixed frame.  This explains the
reversed sandwiches relative to the coordinate actions
\eqref{eq:rotation-action} and \eqref{eq:boost-action}.
For the electric case with $qE>0$, $X(0)=0$, and $P(0)=m$, one has
$U=T^2$.  Integrating gives \eqref{eq:hyperbolic-worldline} with
$a=qE/m$~\cite[p.~117]{tong-em}.
Complex field factors thus already generate real motion.
...
Recall the real proper velocity $U=\dd X/\dd s$, a
\hyperref[eq:unit-paravector]{unit paravector} with $UU^{-1}=1$.
Keep real charges $q,q'$ and complex electromagnetic field $F$ as above,
but postulate a complex paravector $P(s)$ with
\begin{equation}
 \frac{\dd P}{\dd s}=(q+\ii q')FU.
 \label{eq:complex-force-proposal}
\end{equation}
Extracting real and imaginary parts gives
\begin{align*}
...
Thus the identification $P=mU$ with real $U$ is inconsistent in general.
If $P$ is independent of the
real trajectory, these equations only accumulate two impulses; they do
not prescribe a complex trajectory.

To close a complex model, replace the real tangent $U$ by $P/m$,
with complex $P$ and real $m>0$.  Now $s\in\R$ is an evolution parameter,
...
\end{equation}
Take $q'=0$, $q\ne0$, $\bE=0$, and constant $\bBf=B\bu$ with
real $B\ne0$ and arbitrary real unit $\bu$.  With $P(0)=m$, this gives
\begin{equation}
 P(s)=m\cos(qBs/m)+\ii m\sin(qBs/m)\bu\cdot\bsig.
 \label{eq:complex-magnetic-counterexample}
\end{equation}
Integrating for a complex
coordinate $Z(s)$ defined by $\dd Z/\dd s=P/m$ and
$Z(0)=0$ gives
\begin{equation}
 Z(s)=\frac{m}{qB}\sin(qBs/m)
 +\ii\frac{m}{qB}\bigl(1-\cos(qBs/m)\bigr)\bu\cdot\bsig.
...
\end{equation}
Complex nullity requires both
\[
 \det X=\det Y,\qquad tu=\br\cdot\bm w.
\]
For arbitrary perpendicular real unit vectors $\bu,\ba$,
$Z=(\bu+\ii\ba)\cdot\bsig$ is complex-null but its real part
...
Assign proposed real masses $m_0,m_1\geq0$ to its scalar and vector
components:
\begin{align*}
 (\Box+m_0^2)a&=0,\\
 (\Box+m_1^2)\bA&=0.
\end{align*}
For $L=\exp(\theta\bu\cdot\bsig/2)$, with arbitrary real unit $\bu$,
define $\beta=\tanh\theta$ and $\gamma=\cosh\theta$ for real
rapidity $\theta$.  The passive boost gives
\begin{align*}
 a'&=\gamma(a-\beta\bu\cdot\bA).
\end{align*}
\begin{equation}
 (\Box'+m_0^2)a'=-\gamma\beta(m_0^2-m_1^2)\bu\cdot\bA.
 \label{eq:component-mass-test}
\end{equation}
Covariance for arbitrary fields forces $m_0^2=m_1^2$, and similarly
...
The resulting equations and their diagonalizing real combinations are
\begin{align*}
 (\Box+m^2)a+hb&=0,\\
 (\Box+m^2)b+ha&=0,\\
 a'&=(a+b)/\sqrt2,\\
...
\end{align}
The squared masses are $m^2\pm h$, both nonnegative when $|h|\leq m^2$.
...
and chiral structure~\cite[pp.~180--182]{tong-electroweak}.
...
\end{equation}
This density is unbounded below: the independent $b$ mode is a ghost, whose
negative kinetic sign persists at arbitrarily small coupling~\cite[Secs.~III--V]{carroll-ghosts}.
...
Its stress tensor gives~\cite[pp.~34--35]{carroll2001}
\begin{align}
...
Neglect expansion over one cycle and take $\phi=A\cos(mt)$ with real
constant amplitude $A$.  Cycle averaging gives
\begin{align*}
 \left\langle\tfrac12(\partial_t\phi)^2\right\rangle
 &=\langle V\rangle=m^2A^2/4,\\
 \langle p\rangle&=0.
\end{align*}
This limit behaves as pressureless matter.  Neglecting kinetic energy
instead gives $p=-u$.  For the real positive cosmic scale factor $a(t)$ and
Newton's constant $G>0$, the acceleration equation is~\mbox{\cite[p.~6]{carroll2001}}
\begin{equation}
 \frac{\partial_t^2a}{a}=-\frac{4\pi G}{3}(u+3p).
 \label{eq:cosmic-acceleration}
\end{equation}
Acceleration therefore requires $u+3p<0$,
...
\end{equation*}
For real electric and magnetic charges $q,q'$, the force is
\begin{equation*}
...
the complex \hyperref[eq:unit-paravector]{unit paravector}
\begin{equation*}
 T(s)=\exp\!\left(\frac{(q-\ii q')s}{2m}F\right).
...
D. Tong, \emph{Gauge Theory}, University of Cambridge, Cambridge, U.K.,
lecture notes, 2018, ch.~1, ``Topics in electromagnetism.''
[Online]. Available: \url{https://www.damtp.cam.ac.uk/user/tong/gaugetheory/1em.pdf}.

\bibitem{tong-electroweak}
D. Tong, ``Electroweak interactions,'' in \emph{The Standard Model},
University of Cambridge, Cambridge, U.K., lecture notes, ch.~5.
Accessed: Sep.~14, 2026. [Online]. Available:
\url{https://www.damtp.cam.ac.uk/user/tong/sm/standardmodel5.pdf}.
...
\emph{Phys. Rev. D}, vol.~68, no.~2, Jul.~2003, Art.~no.~023509,
doi: \href{https://doi.org/10.1103/PhysRevD.68.023509}{10.1103/PhysRevD.68.023509}.
[Online]. Available: \url{https://arxiv.org/pdf/astro-ph/0301273}.

\bibitem{carroll2001}
S. M. Carroll, ``The cosmological constant,''
\emph{Living Rev. Relativity}, vol.~4, Feb.~2001, Art.~no.~1,
doi: \href{https://doi.org/10.12942/lrr-2001-1}{10.12942/lrr-2001-1}.
