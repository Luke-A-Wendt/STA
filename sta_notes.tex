...
These formulas describe changes of frame along a curve.  They remain useful
in curved spacetime, but curvature requires comparing transport in independent
directions, as developed next.

\subsection{Curved spacetime in the same algebra}
\label{sec:curved-spacetime}

\subsubsection{Local intervals and transport}

Here $X=t+\br\cdot\bsig$ labels real coordinates on an open domain in
$\R^4$; it is not assumed to be a global inertial position.
Let $A=a_0+\ba\cdot\bsig$ and $B=b_0+\bb\cdot\bsig$ be constant real
paravectors specifying coordinate directions, with $a_0,b_0\in\R$ and
$\ba,\bb\in\R^3$.  Define the directional derivative, acting on field
coefficients, by
\[
 \partial_A:=\scpart{A\partial}
 =a_0\partial_t+\ba\cdot\pdr.
\]
Let $H(X,A)$ be a smooth real-paravector-valued function, linear over
$\R$ and invertible in $A$ at each $X$.  Write $H(A)$ when the coordinate
argument is understood.  This \emph{local frame map} assigns the physical
increment $H(\dd X)$ to a coordinate increment $\dd X$ and defines
\begin{equation}
 \boxed{\dd s^2=\det(H(\dd X)).}
 \label{eq:curved-interval}
\end{equation}
The determinant retains one positive and three negative directions.
Every smooth metric of this signature has such a representation locally.
The map $H$ need not preserve determinants: it specifies the metric rather
than changing an observer.  In particular, $H(\dd X)$ need not be the
differential of a single position function, as it was in
Section~\ref{sec:generalized}.

To compare real paravectors at neighboring coordinates, introduce a smooth
complex pure-vector paravector $\Omega(X,A)$, linear over $\R$ in $A$;
write $\Omega(A)$ with $X$ understood.  Its three complex components specify
infinitesimal boosts and rotations.  For a real paravector field $Z(X)$,
define its \emph{covariant derivative} by the existing product:
\begin{equation}
 \nabla_A Z:=\partial_A Z-\Omega(A)^{\mathsf H}Z-Z\Omega(A).
 \label{eq:curved-derivative}
\end{equation}
It measures change after subtracting local transport, following
\eqref{eq:varying-congruence}.  Since $\scpart{\Omega(A)}=0$, direct
expansion gives, for real paravector fields $Z,Y$,
\begin{equation}
 \partial_A\scpart{Z^*Y}
 =\scpart{(\nabla_A Z)^*Y}+\scpart{Z^*\nabla_A Y}.
 \label{eq:curved-metric-compatibility}
\end{equation}
Thus parallel transport, defined by vanishing covariant derivative,
preserves the metric pairing~\eqref{eq:lorentz-pairing} and the determinant.
To make this transport the one determined by $H$, impose
\begin{equation}
 \boxed{\nabla_A H(B)=\nabla_B H(A).}
 \label{eq:curved-torsion-free}
\end{equation}
This condition is called \emph{torsion-free}: the covariant changes of
the two coordinate directions agree when their order is exchanged.
For an invertible $H$, it uniquely determines $\Omega$ together with
metric compatibility~\eqref{eq:curved-metric-compatibility}
\cite[pp.~108--110, 132--136]{tong-gr}.
Computationally, evaluate \eqref{eq:curved-torsion-free} on the six distinct
pairs from $1,\sigma_1,\sigma_2,\sigma_3$: these give 24 real linear
equations for the 24 real coefficients of the map $\Omega$.

Equation~\eqref{eq:curved-derivative} acts on real spacetime paravectors.
A complex field $F$ transforming by similarity instead uses
$\partial_A F-[\Omega(A)^{\mathsf H},F]$, as in \eqref{eq:varying-similarity}.
The two actions follow their respective transformation laws; they cannot
be interchanged when differentiating products.

\subsubsection{Curvature and changes of local frame}

Define the complex pure-vector paravector $C(X,A,B)$, abbreviated $C(A,B)$,
by
\begin{equation}
 C(A,B):=\partial_A\Omega(B)-\partial_B\Omega(A)
          +[\Omega(A),\Omega(B)].
 \label{eq:curved-curvature}
\end{equation}
It is linear over $\R$ in each direction and obeys $C(B,A)=-C(A,B)$.
Expanding the two derivatives in \eqref{eq:curved-derivative} cancels all
derivatives of $Z$ and gives
\begin{equation}
 \boxed{[\nabla_A,\nabla_B]Z=-C(A,B)^{\mathsf H}Z-ZC(A,B).}
 \label{eq:curved-commutator}
\end{equation}
This is the \emph{curvature}: transport around a small coordinate loop
can change a vector even though its determinant is preserved.  With
\eqref{eq:curved-torsion-free}, it is the usual spacetime curvature in
paravector form; see \cite[pp.~123--127, 135--136]{tong-gr}.
The constant-direction assumption is essential to
\eqref{eq:curved-curvature} and \eqref{eq:curved-commutator}.

For a smooth complex paravector $T(X)$ of determinant one, change the
local frame by $Z'=T^{\mathsf H}ZT$ and $H'(A)=T^{\mathsf H}H(A)T$.
The product rule requires
\begin{equation}
 \Omega'(A)=T^{-1}\Omega(A)T+T^{-1}\partial_A T.
 \label{eq:curved-connection-transform}
\end{equation}
Substitution of \eqref{eq:curved-connection-transform} in \eqref{eq:curved-derivative} and
\eqref{eq:curved-curvature} then yields
\begin{align*}
 \nabla_A'Z'&=T^{\mathsf H}(\nabla_A Z)T,\\
 C'(A,B)&=T^{-1}C(A,B)T.
\end{align*}
Hence a frame change preserves \eqref{eq:curved-interval} and cannot
remove nonzero curvature.

In contrast, if one extends the earlier construction to a single smooth
$T(X)$ and sets $\Omega(A)=T^{-1}\partial_A T$, commuting coordinate
derivatives give
\[
 \partial_A\Omega(B)-\partial_B\Omega(A)
 =-[\Omega(A),\Omega(B)],
 \qquad C(A,B)=0.
\]
Thus a varying Lorentz frame alone describes flat transport.  Along one
curve, even a curved connection can be written as
$\Omega=T^{-1}(\dd T/\dd s)$ by solving $\dd T/\dd s=T\Omega$.
In curved spacetime this transport factor depends on the path, not only
its endpoint.  This is the distinction missing from a one-parameter
frame calculation.

\subsubsection{Free fall}

Let $X(s)$ be a future timelike coordinate curve, with proper time $s$
from \eqref{eq:curved-interval}.  Its real local tangent is
\[
 Z(s):=H\left(\frac{\dd X}{\dd s}\right),\qquad
 \det Z=1,\qquad \scpart Z>0.
\]
Here $Z$ represents physical velocity in the local frame; generally
$Z\ne\dd X/\dd s$.  Along the curve, define
\[
 \nabla_s Z:=\frac{\dd Z}{\dd s}
 -\Omega\left(\frac{\dd X}{\dd s}\right)^{\mathsf H}Z
 -Z\Omega\left(\frac{\dd X}{\dd s}\right).
\]
Extend the free-particle action~\eqref{eq:free-particle-action} to the
local interval, with constant real $m>0$.  Vary the timelike path with
fixed endpoints before choosing $s$ as the parameter of the unvaried
curve.  Using \eqref{eq:curved-torsion-free} to exchange the path and
variation directions, then \eqref{eq:curved-metric-compatibility} to
integrate by parts, gives
\begin{align*}
 \delta\left(-m\int\dd s\right)
 &=-m\int\scpart{Z^*\nabla_s(H(\delta X))}\dd s\\
 &=m\int\scpart{H(\delta X)^*\nabla_s Z}\dd s.
\end{align*}
The real variation $\delta X$ is arbitrary and $H$ is invertible, so
stationarity is equivalent to the geodesic, or free-fall, equation
\begin{equation}
 \boxed{\nabla_s Z=0.}
 \label{eq:curved-geodesic}
\end{equation}
Metric compatibility preserves $\det Z=1$.  This is the standard
parallel-transport description of free fall
\cite[Sec.~II, Eqs.~(2.10)--(2.12)]{francis-kosowsky}.

\subsubsection{Acceleration versus curvature}

Choose any constant \hyperref[def:real-unit-vector]{real unit vector}
$\bu$, let $z=\bu\cdot\br$, and let $f(z)>0$ be a smooth real scalar
function on the coordinate domain.  Here $f'=\dd f/\dd z$ and
$f''=\dd^2f/\dd z^2$.
For the constant directions $A,B$ defined above, take
\begin{align*}
 H(A)&=f(z)a_0+\ba\cdot\bsig,\\
 \Omega(A)&=-\tfrac12a_0f'(z)\bu\cdot\bsig.
\end{align*}
The interval is
\begin{equation}
 \dd s^2=f(z)^2\dd t^2-\dd\br^2.
 \label{eq:curved-lapse-interval}
\end{equation}
To check \eqref{eq:curved-torsion-free}, expand the product:
\[
 \nabla_A H(B)
 =f'(b_0\bu\cdot\ba+a_0\bu\cdot\bb)
  +a_0b_0ff'\bu\cdot\bsig.
\]
This is symmetric in $A,B$.  The two $\Omega$ values commute, so
\eqref{eq:curved-curvature} reduces to
\begin{equation}
 C(A,B)=\tfrac12f''(z)
 (a_0\bu\cdot\bb-b_0\bu\cdot\ba)\bu\cdot\bsig.
 \label{eq:curved-lapse-curvature}
\end{equation}
For $f(z)=1+az$, with real constant $a$, this curvature vanishes even
when $a\ne0$: the varying clock rate describes accelerated coordinates
in flat spacetime.  Wherever $f''\ne0$, \eqref{eq:curved-lapse-curvature}
is nonzero for some $A,B$, so the metric is curved.

For this example,
$Z=f(\dd t/\dd s)+(\dd\br/\dd s)\cdot\bsig$.
The scalar and vector parts of \eqref{eq:curved-geodesic} are
\begin{align}
 f\frac{\dd^2t}{\dd s^2}
 +2f'\frac{\dd t}{\dd s}\frac{\dd z}{\dd s}&=0,
 \label{eq:curved-lapse-time}\\
 \frac{\dd^2\br}{\dd s^2}
 +ff'\left(\frac{\dd t}{\dd s}\right)^2\bu&=0.
 \label{eq:curved-lapse-space}
\end{align}
Equations~\eqref{eq:curved-lapse-time} and \eqref{eq:curved-lapse-space}
also follow directly by varying the interval~\eqref{eq:curved-lapse-interval}.
The example specifies a geometry and its free motion; no matter source
has been selected.

\subsubsection{What remains for general relativity}

The frame map, compatible transport, curvature, and free fall provide a
local geometric foundation for GR entirely through the existing
paravector products.  Local determinant preservation fixes the interval
under frame changes; it does not fix the geometry $H$.
Einstein's equation is the additional physical relation between curvature
and matter and energy.  Developing it requires defining the relevant
curvature contractions and matter action, then deriving the field equation.
It does not follow from a varying frame alone.  An algebraic formulation
of standard GR is available; see \cite[Sec.~II, Eqs.~(2.22)--(2.45)]{francis-kosowsky}.
The electromagnetism and wave equations below retain their stated flat
inertial-coordinate scope.
...
let $\psi'=T\psi$.  The pairing gives the same congruence form as
\eqref{eq:general-lorentz-congruence}:
\begin{equation}
 \boxed{\langle\psi'\mid Z\mid\psi'\rangle
 =\scpart{\psi^{\mathsf H}T^{\mathsf H}ZT\psi}
 =\langle\psi\mid T^{\mathsf H}ZT\mid\psi\rangle.}
 \label{eq:amplitude-pullback}
\end{equation}
Thus $T^{\mathsf H}ZT$ evaluates the transformed state's measurement
using the original state.  Dividing by its transformed norm gives
\begin{equation*}
 \langle Z\rangle_{\psi'}
 =\frac{\langle T^{\mathsf H}ZT\rangle_\psi}
        {\langle T^{\mathsf H}T\rangle_\psi}.
\end{equation*}
The denominator is $1$ for the unitary rotation
$R=\exp(-\ii\theta\bu\cdot\bsig/2)$ and $\langle L^2\rangle_\psi$
for the Hermitian boost $L=\exp(-\theta\bu\cdot\bsig/2)$, with real
$\theta$ and real unit $\bu$ as in Section~\ref{sec:preserved-quantities}.
...
\[
 \psi_\pm(0,\br)=\tfrac12\left(\psi(0,\br)
             \pm\ii\Omega^{-1}\partial_t\psi(0,\br)\right).
\]
...
\end{equation*}
This gives the nonnegative real density
\begin{equation}
 \lVert\bPsi\rVert^2
...
integrated pairing, so $\int\lVert\bPsi\rVert^2\dd^3\br$ is conserved,
with density \eqref{eq:dirac-norm}.
...
\begin{align*}
 Ea_2-\bp\cdot\bb_2&=ma_1,\\
 E\bb_2-\bp a_2-\ii\bp\times\bb_2&=m\bb_1,\\
 Ea_1+\bp\cdot\bb_1&=ma_2,\\
 E\bb_1+\bp a_1+\ii\bp\times\bb_1&=m\bb_2.
\end{align*}
...
Recall a Pauli amplitude $\psi$ with $\lVert\psi\rVert=1$ and its real polarization
$\bm{s}$ in \eqref{eq:spin-state}.  Write $\langle Z\rangle$ for
$\langle Z\rangle_\psi$ from \eqref{eq:paravector-expectation}.
...
Local curved intervals, compatible transport, curvature, and free fall
are \eqref{eq:curved-interval}, \eqref{eq:curved-derivative},
\eqref{eq:curved-torsion-free}, \eqref{eq:curved-commutator}, and
\eqref{eq:curved-geodesic}; their scope is Section~\ref{sec:curved-spacetime}.
...
The pairing and measurement formulas are
\begin{align*}
...
\end{align*}
Here $\psi'$ is a second paravector amplitude.
...
For real $a$ and $\bA\in\R^3$, define the quaternion
\begin{equation*}
 Q=a+\bA\cdot\bq=a-\ii\bA\cdot\bsig.
\end{equation*}
It is a complex paravector with real quaternion coordinates.
...
\]
Then $Q=R$ for $R=\exp(-\ii\theta\bu\cdot\bsig/2)$.
For a real vector $\br$, Rodrigues' formula~\eqref{eq:rodrigues}
in quaternion form is
\begin{equation*}
...
\bibitem{tong-gr}
D. Tong, \emph{General Relativity}, University of Cambridge,
Cambridge, U.K., lecture notes, 2019.
[Online]. Available: \url{https://www.davidtong.org/pdfs/teaching/general-relativity/gr.pdf}.

\bibitem{francis-kosowsky}
M. R. Francis and A. Kosowsky,
``Geometric algebra techniques for general relativity,''
\emph{Ann. Phys.}, vol.~311, no.~2, pp.~459--502, 2004,
doi: \href{https://doi.org/10.1016/j.aop.2003.12.009}{10.1016/j.aop.2003.12.009}.
[Online]. Available: \url{https://arxiv.org/pdf/gr-qc/0311007}.
