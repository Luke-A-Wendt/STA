...
\begin{equation}
 \boxed{X'=T^{\mathsf H}XT.}
 \label{eq:general-lorentz-congruence}
\end{equation}
Hermitian conjugation and determinant multiplication give
\begin{align*}
 X'^{\mathsf H}&=T^{\mathsf H}X^{\mathsf H}T=X',\\
 \det(X')&=\det(T^{\mathsf H})\det(X)\det(T)=\det(X).
\end{align*}
...
\[
 \scpart{X'}=\lVert\sqrt X\,T\rVert^2\geq0.
\]
...
\begin{align*}
 X'&=L^{\mathsf H}L=L^2=\cosh\theta-\sinh\theta\bu\cdot\bsig,\\
 \det(X')&=1,\\
 \scpart{X'}&=\cosh\theta,\\
...
$X=1$: the real, positive $T^{\mathsf H}T$ can have both determinant and
norm one only if $T^{\mathsf H}T=1$.  Sufficiency was proved above.
Normalizing $L$ to $\lVert L\rVert=1$ would divide the determinant of
$L^{\mathsf H}XL$ by $\cosh^2\theta$, losing the Lorentz metric.

\paragraph{Every factor separates into a boost and a rotation.}
For a determinant-one complex $T$, the paravector $T^{\mathsf H}T$
...
\[
 B:=\sqrt{T^{\mathsf H}T},\qquad R:=TB^{-1}.
\]
Then $\det(B)=1$,
$R^{\mathsf H}R=1$, $\det(R)=1$, and $T=RB$.
...
Consequently $T^{\mathsf H}XT=BR^{\mathsf H}XRB$: the rotation is
...
\begin{equation}
 ...
 \label{eq:rodrigues}
\end{equation}
These are the components in axes rotated right-handed through $+\theta$
about $\bu$; the components themselves rotate through $-\theta$.
...
For a unit quaternion $Q=a+\bA\cdot\bq$, use the same order as
\eqref{eq:rotation-action}:
\begin{equation}
 \br'\cdot\bq=Q^{\mathsf H}(\br\cdot\bq)Q,
 \qquad
 \br'\cdot\bsig=Q^{\mathsf H}(\br\cdot\bsig)Q.
...
\begin{align*}
 Q^{\mathsf H}(\br\cdot\bq)
 &=\bA\cdot\br+(a\br-\bA\times\br)\cdot\bq,\\
 Q^{\mathsf H}(\br\cdot\bq)Q
...
$\bV=\br$, with components in axes rotated through $+\theta$.
The active rotation of $\br$ through $+\theta$ instead uses
$Q(\br\cdot\bq)Q^{\mathsf H}$ and has a plus sign on the cross term.
...
\begin{align*}
 L^{\mathsf H}(S+\bV_{\parallel}\cdot\bsig)L
 &=(S+\bV_{\parallel}\cdot\bsig)L^2,\\
 L^{\mathsf H}(\bV_{\perp}\cdot\bsig)L
 &=\bV_{\perp}\cdot\bsig.
\end{align*}
...
\paragraph{Equivalent factors and unsuccessful alternatives.}
The positive-exponent definition is equally valid: setting
$L':=L^{-1}=\exp(+\theta\bu\cdot\bsig/2)$ uses $T=(L')^{-1}$ in
\eqref{eq:general-lorentz-congruence}.  The negative exponent uses $T=L$
directly in \eqref{eq:boost-action}.
The full-rapidity factor $L^2=\gamma(1-\beta\bu\cdot\bsig)$ has the
familiar gamma prefactor, but requires
$Z'=(\sqrt{L^2})^{\mathsf H}Z\sqrt{L^2}$,
using the positive real square root.  Using $L^2$ on both sides instead
would double the rapidity.  A one-sided product $L^2Z$ preserves the
determinant but generally adds an imaginary vector proportional to
$\bu\times\bV$; it cannot transform real spacetime.
...
$L=\exp(-\theta\bu\cdot\bsig/2)$: the direct boost $L^{\mathsf H}UL=1$
...
$Z'=T^{\mathsf H}Z(T^{\mathsf H})^{-1}$, product differentiation yields
\begin{equation}
 \frac{\dd Z'}{\dd s}
 =T^{\mathsf H}\frac{\dd Z}{\dd s}(T^{\mathsf H})^{-1}+[\Omega^{\mathsf H},Z'].
 \label{eq:varying-similarity}
\end{equation}
Differentiating again and replacing
$T^{\mathsf H}(\dd Z/\dd s)(T^{\mathsf H})^{-1}$ by its first-derivative expression gives
\begin{equation*}
 \begin{aligned}
 \frac{\dd^2 Z'}{\dd s^2}={}&T^{\mathsf H}\frac{\dd^2Z}{\dd s^2}(T^{\mathsf H})^{-1}
 +2\left[\Omega^{\mathsf H},\frac{\dd Z'}{\dd s}\right]\\
 &+\left[\frac{\dd\Omega^{\mathsf H}}{\dd s},Z'\right]
 -[\Omega^{\mathsf H},[\Omega^{\mathsf H},Z']].
 \end{aligned}
\end{equation*}
For a real paravector transformed instead by congruence,
$Z'=T^{\mathsf H}ZT$, the same rule gives
\begin{equation}
 \frac{\dd Z'}{\dd s}
 =T^{\mathsf H}\frac{\dd Z}{\dd s}T
 +\Omega^{\mathsf H}Z'+Z'\Omega.
...
$\sigma_k'=T^{\mathsf H}\sigma_k(T^{\mathsf H})^{-1}$, with
\begin{equation*}
 \frac{\dd\sigma_k'}{\dd s}=[\Omega^{\mathsf H},\sigma_k'],\qquad k=1,2,3.
\end{equation*}
...
$F'=T^{\mathsf H}F(T^{\mathsf H})^{-1}$ preserves this scalar by \eqref{eq:det-similarity}.
...
For complex paravectors $\psi,\psi'$, the mixed scalar
$\scpart{\psi'^{\mathsf H}\psi}$ is their algebraic inner product.
We write this product directly and reserve angle brackets for normalized
expectations below.
...
set $\psi'=T\psi$ and $Z'=T^{\mathsf H}ZT$.
For a nonnegative weight $Z$, substitution gives
\begin{equation}
 \boxed{\lVert\psi'\rVert_Z^2
 =\lVert T\psi\rVert_Z^2
 =\scpart{\psi^{\mathsf H}Z'\psi}
 =\lVert\psi\rVert_{Z'}^2.}
 \label{eq:weighted-density-pullback}
\end{equation}
This also proves that $Z'$ is nonnegative.
The same substitution in \eqref{eq:paravector-expectation} applies to
any complex $Z$.  With $\rho'=\lVert\psi'\rVert^2>0$, it gives
\begin{equation}
 \boxed{\langle Z\rangle_{\psi'}
 =\frac{\rho}{\rho'}\langle Z'\rangle_\psi.}
 \label{eq:amplitude-pullback}
\end{equation}
Thus the congruence form in \eqref{eq:general-lorentz-congruence} arises
directly from the same local product.  For real $\theta$ and real unit $\bu$,
$R=\exp(-\ii\theta\bu\cdot\bsig/2)$ is unitary and gives $\rho'=\rho$.
For the Hermitian boost $L=\exp(-\theta\bu\cdot\bsig/2)$,
$Z=1$ gives $Z'=L^2$ and $\rho'=\lVert\psi\rVert_{Z'}^2$,
which generally differs from $\rho$.
...
\end{equation*}
This is exactly the rotation action~\eqref{eq:rotation-action} inside
the original state's pairing.
...
\end{equation*}
Rodrigues' formula~\eqref{eq:rodrigues} rotates the measurement axis
through $-\theta$, so $\bm{s}'$ rotates through $+\theta$ about $\ba$.
...
\begin{align}
 \partial'&=T^{-1}\partial(T^{-1})^{\mathsf H},
 \label{eq:derivative-transformation}\\
 F'&=T^{\mathsf H}F(T^{\mathsf H})^{-1}. \label{eq:field-transformation}
\end{align}
...
\[
 ...
 =\scpart{\bigl(T^{-1}(\partial f)(T^{-1})^{\mathsf H}\bigr)\dd X'}.
\]
...
\begin{equation*}
 \partial'F'=T^{-1}(\partial F)(T^{-1})^{\mathsf H}
 =T^{-1}D^*(T^{-1})^{\mathsf H}=D'^*.
\end{equation*}
...
gives $X'=L^{\mathsf H}XL$: the primed frame moves at $+\beta\bu$.
...
\begin{equation}
 \dd X'=L^{\mathsf H}\dd X L.
 \label{eq:boost-worldline-step}
\end{equation}
Because $\dd s$ is invariant, $U'=L^{\mathsf H}UL$.  At a point where
...
\begin{equation*}
 X'(s_2)-X'(s_1)
 =L^{\mathsf H}\left(\int_{s_1}^{s_2}U(s)\dd s\right)L.
\end{equation*}
...
$T$ and then $T'$ gives
\begin{equation*}
 Z''=T'^{\mathsf H}(T^{\mathsf H}ZT)T'
 =(TT')^{\mathsf H}Z(TT').
\end{equation*}
Thus the right factors multiply in the order the frames are applied.
...
\begin{equation*}
 P=(L^{-1})^{\mathsf H}mL^{-1}=m\gamma(1+\beta\bu\cdot\bsig).
\end{equation*}
...
$\partial'=L^{-1}\partial(L^{-1})^{\mathsf H}$.  Its scalar and vector parts are
\begin{align*}
...
\begin{equation}
 F'(X')=L F((L^{-1})^{\mathsf H}X'L^{-1})L^{-1}.
 \label{eq:boosted-field-arguments}
\end{equation}
Here $X=(L^{-1})^{\mathsf H}X'L^{-1}$.  To expand \eqref{eq:boost-field-action},
...
here $T$ denotes this map.  Since $N'=L^{\mathsf H}NL$,
\begin{equation*}
 T'(N')=\tfrac12F'N'F'^{\mathsf H}=L^{\mathsf H}T(N)L.
\end{equation*}
Here $T'$ is the map formed from $F'$.  To measure the primed
energy density and Poynting vector, use $N'=1$, hence $N=L^{-2}$:
\begin{equation*}
 T'(1)=L^{\mathsf H}T(L^{-2})L=\tfrac12F'F'^{\mathsf H}.
\end{equation*}
...
With $U'=L^{\mathsf H}UL$ and $P'=L^{\mathsf H}PL$, the force
law~\eqref{eq:lorentz} transforms as
\begin{equation*}
 \frac{\dd P'}{\dd s}
 =L^{\mathsf H}\frac q2(FU+UF^{\mathsf H})L
...
\end{align*}
Both $Z$ transformations use $Z'=T^{\mathsf H}ZT$, with
...
\begin{align*}
 \operatorname{mat}(X')&=\operatorname{mat}(L)^{\mathsf H}
