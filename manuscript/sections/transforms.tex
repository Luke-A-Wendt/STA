...
\end{align}
\noindent\textit{Applying the sigma-conjugate and reverse operators gives}\par
\begin{align}
( \sigma _{n}' )^{*} &= (T^{-1})^{*} \sigma _{n}^{*} T^{*},\notag \\
&= -(T^{*})^{-1} \sigma _{n} T^{*}.
\end{align}
...
\end{align}
\noindent\textit{Define the following object.}\par
\begin{align}
\Omega &:= T^{-1} (dT/dt),\notag \\
...
\end{align}
\noindent\textit{Energy \& momentum.}\par
\begin{align}
m dX/dt &= E + \mathbf P' \cdot \sigv ', \\
E / m &= dt/dt, \\
\mathbf P' / m &= d\mathbf r/dt + \mathbf r \times (-\omega + i k).
\end{align}
If \(T^{-1} = T^{*\mathrm{R}},\), then the following relation holds.\par
\begin{align}
...
\noindent\textit{Commutative over the same axis.}\par
\begin{equation}
[ L(\theta ,\mathbf u), L(\theta ',\mathbf u) ] = 0.
\end{equation}
\noindent\textit{Co-linear boost.}\par
\begin{align}
...
\end{align}
\noindent\textit{Re-mapping space-time numbers.}\par
\begin{align}
X &= t + \mathbf r \cdot \sigv,\notag \\
X' &= L^{*} X L,\notag \\
&= t' + \mathbf r' \cdot \sigv.
...
\end{align}
Split the vector \(\mathbf r\) into parts parallel and perpendicular to the boost direction.\par
\noindent\textit{Parallel and perpendicular to u.}\par
\begin{align}
...
\end{align}
If \(\beta \Rightarrow 0,\), then the following relation holds.\par
\begin{equation}
\gamma \Rightarrow 1, t' \Rightarrow t, \mathbf r' \Rightarrow \mathbf r.
\end{equation}
\noindent\textit{Paravector metric invariance.}\par
\begin{align}
...
\end{align}
\noindent\textit{Differentiating the transformed quantities gives.}\par
\begin{align}
...
dR &= -R dR^{\mathrm{R}} R.
\end{align}
\noindent\textit{The reverse symmetry can be written as.}\par
\begin{align}
