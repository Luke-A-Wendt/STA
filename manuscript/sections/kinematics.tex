...
\eqtext{scalar}(\pv{U})&=\gamma,\qquad
\eqtext{vector}(\pv{U})=\gamma\vct{v}.
\end{align*}
...
hence
\begin{equation*}
\eqtext{scalar}\!\left(\frac{\dd\pv{U}}{\dd s}\pv{U}^*\right)
=\eqtext{scalar}\!\left(\pv{U}^*\frac{\dd\pv{U}}{\dd s}\right)
=0.
\end{equation*}
...
\(\eqtext{scalar}(\pv{U})=\gamma\) and
\(\eqtext{vector}(\pv{U})=\gamma\vct{v}\), as obtained above.
...
\pv{U}\pv{A}
&=\bigl(
\eqtext{scalar}(\pv{U})\eqtext{scalar}(\pv{A})
+\eqtext{vector}(\pv{U})\cdot\eqtext{vector}(\pv{A})
\bigr)\notag\\
&\quad+\bigl(
\eqtext{scalar}(\pv{U})\eqtext{vector}(\pv{A})
+\eqtext{scalar}(\pv{A})\eqtext{vector}(\pv{U})\notag\\
&\qquad\qquad
+i\bigl(\eqtext{vector}(\pv{U})\times\eqtext{vector}(\pv{A})\bigr)
\bigr)\cdot\sigv,\\
\pv{U}^*\pv{A}
&=\bigl(
\eqtext{scalar}(\pv{U})\eqtext{scalar}(\pv{A})
...
one number is assigned to the complete path by the integral
\begin{equation*}
\int \mathcal L(t,\vct{r},\vct{v})\,\dd t.
\end{equation*}
This number is called the action, and its integrand \(\mathcal L\) is
called the Lagrangian. Requiring the first-order change of the action to
...
corresponding dependence. The derivative with respect to velocity,
\begin{equation*}
\frac{\partial\mathcal L}{\partial\vct{v}},
\end{equation*}
is called canonical momentum. If the Lagrangian has no position dependence,
...
\subsubsection{Flat Interval in General Coordinates}
The map \(\pv{X}=f+\vct{g}\cdot\sigv\) permits the same flat interval to be
written in terms of the original four coordinate labels
\[
\vct{x}=\vcol{t,r_1,r_2,r_3},
\qquad
\dd\vct{x}=\vcol{\dd t,\dd r_1,\dd r_2,\dd r_3}.
\]
...
\[
\Qmat=\Nmat\eqtext{diag}(\lambda(1),\lambda(2),\lambda(3),\lambda(4))\mtrans{\Nmat},
\qquad
\mtrans{\Nmat}\Nmat=\I.
\]
The second equality states that the columns have unit length and are
mutually perpendicular. The eigenvalues are ordered numerically so that
\[
