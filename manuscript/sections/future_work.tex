...
The fixed projected two-amplitude space used for one Dirac field is a useful
starting point, but these many-particle constructions are not supplied by it
alone.
Likewise, the two auxiliary projectors in
Eq.\ \eqref{eq:W-projectors} only select the two eigenvalues of the block map
defined in Section \ref{sec:matrix-dirac-packaging}. Their algebraic signs cannot be
identified with particle and antiparticle sectors until charge reversal,
particle-number change, and the physical field space have been constructed.
...
Likewise, the determinant of a differential operator minus a proposed
measured value is not defined merely because the paravector determinant is
defined. A finite determinant whose zeros select possible values may be used
only after the equation has been reduced to finitely many components, for
...
displayed \(\Hop\) operation, that the energy-selecting operators preserve their
...
\(\Hop\) operations exist must also be required. The standard lower bound built
...
block's normalized opposite-order product is Hermitian and unchanged when
squared, so it represents one fully specified two-amplitude state
\cite{sakurai2017}. The remaining
...
is still required before physical meaning is assigned. For a
fixed-projector state and a left-acting operator that preserves that space,
the product
\(\herm{\pv{\psi}_{\eqtext{block}}}
\pv{\mathcal{O}}\pv{\psi}_{\eqtext{block}}\) is already fixed by its one
complex scalar coefficient and contains no additional independent channels.
...
\(\pv{\Pi}(\pm)\) selection functions for a stated unit direction
\(\vct{u}\), and field products should all be
...
that depend on an optional translation collected in an appendix.

\subsection{TODO: Complex Space-Time and Imaginary Coordinates}
A complex paravector \(\pv{Z}\in\CPspace\) is already permitted by the
generic algebra: \(\pv{Z}=\pv{X}+i\pv{Y}\), with
\(\pv{X},\pv{Y}\in\RPspace\). Treating such an element as a physical event, rather than as an
