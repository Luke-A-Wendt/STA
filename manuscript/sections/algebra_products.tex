...
\begin{align}
\operatorname{acom}(\mathbf A\cdot\sigv,\mathbf B\cdot\sigv)
...
\end{align}
So the anticommutator function \(\operatorname{acom}(\cdot,\cdot)\) isolates the symmetric dot-product part, just as the commutator isolates the antisymmetric cross-product part.

More generally, for generic complex paravectors \(Z,Z'\in\C\oplus\C^3\), define
\begin{equation}
\operatorname{acom}(Z,Z')
:=ZZ'+Z'Z.
...
\end{equation}
The cross-product contribution cancels identically, so \(\operatorname{acom}(Z,Z')\) keeps only the symmetric scalar and vector couplings.
